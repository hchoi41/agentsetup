...
# Two naming conventions coexist (fleet v2.2, 2026-08-20):
#   legacy   : {project}_ram_YYYYMMDD_vN.md      -> date after _ram_
#   date-first: YYYYMMDD_{project}_ram_vN.md     -> date leads the filename
RAM_RE_LEGACY = re.compile(r"_ram_(\d{8})_v(\d+)\.md$", re.IGNORECASE)
RAM_RE_DATEFIRST = re.compile(r"^(\d{8})_.*_ram_v(\d+)\.md$", re.IGNORECASE)


def ram_key(name):
    """Return (YYYYMMDD, version) for either naming convention, else None."""
    m = RAM_RE_LEGACY.search(name)
    if not m:
        m = RAM_RE_DATEFIRST.match(name)
    if not m:
        return None
...
def newest_ram_on_disk(root):
    ...
    for name in names:
        key = ram_key(name)  # (YYYYMMDD, version) in either convention
        if key is None:
            continue
        if best_key is None or key > best_key:
            best, best_key = name, key
    if best is None:
        return None, "no RAM files matching either naming convention found"
    return best, None
